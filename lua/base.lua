...
B.gui = {
	...
	CurSearch      = { fg = C.night.c0, bg = C.frost.turquoise, blend = 5 }, --	Used for highlighting a search pattern under the cursor (see 'hlsearch').
	...
	CursorLineNr   = { fg = C.snow.c1, bg = C.none },                      --	Like LineNr when 'cursorline' is set and 'cursorlineopt' contains "number" or is "both", for the cursor line.
	CursorLineSign = { fg = C.snow.c1, bg = C.none },                      --	Like SignColumn when 'cursorline' is set for the cursor line.
	...
	FloatTitle     = { fg = C.night.c3, bg = C.bg },                       --	Title of floating windows.
	FoldColumn     = { fg = C.none, bg = C.none },                         --	'foldcolumn'
	Folded         = { fg = C.none, bg = C.none },                         --	Line used for closed folds.
	IncSearch      = { fg = C.night.c0, bg = C.frost.turquoise, blend = 5 }, --	'incsearch' highlighting; also used for the text replaced with ":s///c".
	LineNr         = { fg = C.snow.c0, bg = C.bg },                        --	Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
	...
	MoreMsg        = { fg = C.frost.turquoise, bg = C.bg, blend = 1 },     --	more-prompt
	...
	NonText        = { fg = C.bg, bg = C.none },                           --	'@' at the end of the window, characters from 'showbreak' and other characters that do not really exist in the text (e.g., ">" displayed when a double-wide character doesn't fit at the end of the line). See also hl-EndOfBuffer.
	Normal         = { fg = C.fg, bg = C.bg, blend = 1 },                  --	Normal text.
	NormalFloat    = { fg = C.fg, bg = C.bg, blend = 1 },                  --	Normal text in floating windows.
	NormalNC       = { fg = C.none, bg = C.bg, blend = 1 },                --	Normal text in non-current windows.
	Pmenu          = { fg = C.night.c3, bg = C.night.c1 },                 --	Popup menu: Normal item.
	PmenuExtra     = { fg = C.none, bg = C.none },                         --	Popup menu: Normal item "extra text".
	PmenuExtraSel  = { fg = C.none, bg = C.none },                         --	Popup menu: Selected item "extra text".
	PmenuKind      = { fg = C.none, bg = C.none },                         --	Popup menu: Normal item "kind".
	PmenuKindSel   = { fg = C.none, bg = C.none },                         --	Popup menu: Selected item "kind".
	PmenuSbar      = { fg = C.none, bg = C.bg },                           --	Popup menu: Scrollbar.
	PmenuSel       = { fg = C.night.c0, bg = C.frost.turquoise, blend = 10 }, --	Popup menu: Selected item.
	PmenuThumb     = { fg = C.none, bg = C.night.c2 },                     --	Popup menu: Thumb of the scrollbar.
	Question       = { fg = C.fg, bg = C.bg, blend = 1 },                  --	hit-enter prompt and yes/no questions.
	QuickFixLine   = { fg = C.none, bg = C.none },                         --	Current quickfix item in the quickfix window. Combined with hl-CursorLine when the cursor is there.
	Search         = { fg = C.night.c0, bg = C.frost.turquoise, blend = 5 }, --	Last search pattern highlighting (see 'hlsearch'). Also used for similar items that need to stand out.
	...
	Substitute     = { fg = C.none, bg = C.frost.turquoise },              --	:substitute replacement text highlighting.
	...
	Visual         = { fg = C.none, bg = C.night.c1, italic = true },      --	Visual mode selection.
	VisualNOS      = { fg = C.none, bg = C.night.c1 },                     --	Visual mode selection when vim is "Not Owning the Selection".
	...
	WinSeparator   = { fg = C.night.c2, bg = C.none },                     --	Separators between window splits.
	...
	DiffText       = { fg = C.snow.c2, bg = C.bg },                        --	Diff mode: Changed text within a changed line. diff.txt
	--Health
	healthError    = { fg = C.aurora.red, bg = C.bg },
	healthSuccess  = { fg = C.aurora.green, bg = C.bg },
	healthWarning  = { fg = C.aurora.yellow, bg = C.bg },
	-- Gui
	Menu           = { fg = C.none, bg = C.none }, -- 	Current font, background and foreground colors of the menus. Also used for the toolbar. Applicable highlight arguments: font, guibg, guifg. hl-Scrollbar
	Scrollbar      = { fg = C.none, bg = C.none }, -- 	Current background and foreground of the main window's scrollbars. Applicable highlight arguments: guibg, guifg. hl-Tooltip
	Tooltip        = { fg = C.none, bg = C.none }, -- 	Current font, background and foreground of the tooltips. Applicable highlight arguments: font, guibg, guifg.
}

B.syntax = {
	-- Lang
	Boolean        = { fg = C.aurora.yellow, bg = C.none }, -- a boolean constant: TRUE, false
	Character      = { fg = C.frost.sea, bg = C.none },     -- any character constant: 'c', '\n'
	...
	Define         = { fg = C.frost.light_blue, bg = C.none }, -- preprocessor #define
	Delimiter      = { fg = C.frost.turquoise, bg = C.none }, -- character that needs attention like , or .
	Error          = { fg = C.aurora.red, bg = C.none },    -- any erroneous construct with bold
	Exception      = { fg = C.frost.light_blue, bg = C.none }, -- try, catch, throw
	...
	Identifier     = { fg = C.snow.c0, bg = C.none },       -- any variable name
	...
	Label          = { fg = C.frost.turquoise, bg = C.none }, -- case, default, etc.
	...
	PreCondit      = { fg = C.none, bg = C.none },          -- preprocessor #if, #else, #endif, etc.
	PreProc        = { fg = C.none, bg = C.none },          -- generic Preprocessor
	Repeat         = { fg = C.frost.light_blue, bg = C.none }, -- italic any other keyword
	Special        = { fg = C.frost.light_blue, bg = C.none }, -- any special symbol
	SpecialChar    = { fg = C.aurora.yellow, bg = C.none }, -- special character in a constant
	SpecialComment = { fg = C.frost.blue, bg = C.none },    -- special things inside a comment
	Statement      = { fg = C.frost.light_blue, bg = C.none }, -- any statement
	StorageClass   = { fg = C.frost.light_blue, bg = C.none }, -- static, register, volatile, etc.
	String         = { fg = C.aurora.green, bg = C.none },  -- any string
	Structure      = { fg = C.aurora.yellow, bg = C.none }, -- struct, union, enum, etc.
	Tag            = { fg = C.aurora.red, bg = C.none },    -- you can use CTRL-] on this
	Todo           = { fg = C.aurora.yellow, bg = C.none }, -- anything that needs extra attention; mostly the keywords TODO FIXME and XXX
	Type           = { fg = C.frost.turquoise, bg = C.none }, -- int, long, char, etc.
	Typedef        = { fg = C.frost.sea, bg = C.none },     -- A typedef
	Underlined     = { fg = C.aurora.yellow, bg = C.none }, -- text that stands out, HTML links
}
